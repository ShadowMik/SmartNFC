...
#ifdef PN532DEBUG
    Serial.print("[SPI PINS] CLK:A3,MISO:A4,MOSI:A5,SS:");
    Serial.println(_ss);
#endif
  }
  else
...
uint8_t Adafruit_PN532::readGPIO(void)
{
  ...
  Serial.println("");
  Serial.print("P3 GPIO: 0x");
  Serial.println(pn532_packetbuffer[6], HEX);
  Serial.print("P7 GPIO: 0x");
  Serial.println(pn532_packetbuffer[7], HEX);
  Serial.print("IO GPIO: 0x");
  Serial.println(pn532_packetbuffer[8], HEX);
...
#ifdef MIFAREDEBUG
  Serial.print("Setting MxRtyPassiveActivation to ");
  Serial.print(maxRetries, DEC);
  Serial.println(" ");
#endif
...
uint8_t Adafruit_PN532::mifareclassic_AuthenticateBlock(uint8_t *uid, uint8_t uidLen, uint32_t blockNumber, uint8_t keyNumber, uint8_t *keyData)
{
  ...
  Serial.print("Using authentication KEY ");
  Serial.print(keyNumber ? 'B' : 'A');
  Serial.print(": ");
...
void Adafruit_PN532::writebyte(uint8_t c)
{
  ...
  for (uint8_t bit = 0; bit < 8; bit++)
  {
    ...
    if (c & (1 << bit))
    {                                                                 // walks up mask from bit 0 to bit 7
...
void Adafruit_PN532::shutDown_PN532()
{
  pn532_packetbuffer[0] = PN532_COMMAND_POWERDOWN;
  pn532_packetbuffer[1] = 0x02;   // wakeupsource 0x02 = INT1
  pn532_packetbuffer[2] = 0x00;   // enable IRQ 0x01, 0x00 dont use   
